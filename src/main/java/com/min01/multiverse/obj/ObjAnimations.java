...
public class ObjAnimations 
{
	public static void animate(WavefrontObject obj, ObjAnimationDefinition p_232321_, long p_232322_, float p_232323_, Vector3f p_253861_) 
	{
		float f = getElapsedSeconds(p_232321_, p_232322_);
		for(Map.Entry<String, List<ObjAnimationChannel>> entry : p_232321_.boneAnimations().entrySet()) 
		{
			List<ObjAnimationChannel> list = entry.getValue();
			list.forEach((p_288241_) ->
			{
				...
				p_288241_.target().apply(obj, p_253861_);
			});
		}
	}

	private static float getElapsedSeconds(ObjAnimationDefinition p_232317_, long p_232318_) 
	{
	...
	{
		return new Vector3f(p_254402_ * ((float) Math.PI / 180.0F), p_253917_ * ((float) Math.PI / 180.0F), p_254397_ * ((float) Math.PI / 180.0F));
	}
