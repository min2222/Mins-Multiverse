	    ...
	    if(!p_115455_.chains.isEmpty())
	    {
	    	for(OrochiChain chain : p_115455_.chains)
	    	{
				for(int i = 1; i < chain.getSegments().length - 1; i++)
				{
					...
					if(firstPos.y < p_115455_.level.getMinBuildHeight() && chain.chainType != ChainType.GIANT)
					{
						continue;
					}
					p_115458_.pushPose();
					...
					p_115458_.scale(chain.scale, chain.scale, chain.scale);
