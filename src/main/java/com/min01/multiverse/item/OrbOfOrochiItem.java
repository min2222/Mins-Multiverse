...
public class OrbOfOrochiItem extends Item
{
	public OrbOfOrochiItem() 
	{
	...
	{
		Vec3 lookPos = MultiverseUtil.getLookPos(new Vec2(entity.getXRot(), entity.getYHeadRot()), entity.getEyePosition(), 0, 0, 300);
		EntityOrochi orochi = new EntityOrochi(MultiverseEntities.OROCHI.get(), level);
		orochi.setPos(entity.getEyePosition());
		orochi.setSnake(true);
		orochi.setChainType(ChainType.SPIRIT);
		orochi.setWantedPos(lookPos);
		orochi.setAnchorPos(entity.getEyePosition());
		orochi.setOwner(entity);
		orochi.setScale(3.0F);
